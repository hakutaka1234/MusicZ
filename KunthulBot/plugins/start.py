from KunthulBot import KunthulBot, BOT_USERNAME
...
@KunthulBot.on(events.NewMessage(pattern="sks"))
async def start(event):
    ...
    if event.is_private:
       await event.client.send_file(event.chat_id,
        ...
        [Button.url("👨‍💻 ꜱᴏᴜʀᴄᴇ ᴄᴏᴅᴇ", "https://github.com/ITZ-KunthulBot/Telethon-Music")],
...
@KunthulBot.on(events.callbackquery.CallbackQuery(data="start"))
async def _(event):
    if Config.MANAGEMENT_MODE == "ENABLE":
        return
    if event.is_private:
